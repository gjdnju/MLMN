...
from utils import load_txt, load_pkl, load_json, label_reflect, pad_sequence, EarlyStopping
from parameters import parse


class MyDataSet(Dataset):
    def __init__(self, corpus, arg):
        vocab = load_pkl(arg.vocab_path)
        stop_words = [line.strip() for line in load_txt(arg.stopwords_path)]
        articles = [item[1].split(':')[1]
                    for item in [line.strip().split('|') for line in load_txt(arg.article_path)]]
        ...
        self.articles = torch.nn.utils.rnn.pad_sequence(sorted_articles,
                                                        batch_first=True)  # (article_num, max_article_len)
        ...
        for data in corpus:
            ...
            if len(tokenized_data) == 0:
                continue
            self.corpus.append((original_data, tokenized_data))
...
class FactEncoder(nn.Module):
    ...
    def forward(self, facts, fact_lens):
        ...
        packed_facts = pack_padded_sequence(sorted_facts, sorted_lens, batch_first=True)
...
class ArticleEncoder(nn.Module):
    ...
    def forward(self, articles, article_lens):
        ...
        :return:
        """
        packed_articles = pack_padded_sequence(articles, article_lens, batch_first=True)
...
class DecisionPredictor(nn.Module):
    def __init__(self, arg):
        ...
        self.embedding = nn.Embedding.from_pretrained(load_pkl(arg.embedding_matrix_path), freeze=True)
...
def train_model(args, device):
    print('Load data...')
    all_data = load_json(args.corpus_path)
    ...
    train_dataset, valid_dataset, test_dataset = \
        MyDataSet(train_data, args), MyDataSet(valid_data, args), MyDataSet(test_data, args)
    train_dataloader, valid_dataloader, test_dataloader = \
        DataLoader(train_dataset, batch_size=args.batch_size, shuffle=True, collate_fn=collate_fn), \
        DataLoader(valid_dataset, batch_size=args.batch_size, collate_fn=collate_fn), \
        DataLoader(test_dataset, batch_size=args.batch_size, collate_fn=collate_fn)
...
def test(model_path, arg):
    device = 'cuda:0' if torch.cuda.is_available() else 'cpu'
    model = DecisionPredictor(arg).to(device)
    model.load_state_dict(torch.load(model_path))

    all_data = load_json(arg.corpus_path)
    all_dataset = MyDataSet(all_data, arg)
    dataloader = DataLoader(all_dataset, batch_size=1, collate_fn=collate_fn)
    with torch.no_grad():
        for i, (facts, fact_lens, artis, arti_lens, fact_indices, arti_indices, labels) in enumerate(dataloader):
            facts, fact_lens, artis, arti_lens, labels = \
                facts.to(device), fact_lens.to(device), artis.to(device), arti_lens.to(device), labels.to(device)
            preds = model(facts, fact_lens, artis, arti_lens, fact_indices, arti_indices)
            preds = torch.argmax(preds.cpu(), dim=-1)
            print(all_data[i]['paragraph'])
            print(str(labels) + '|' + str(preds))
